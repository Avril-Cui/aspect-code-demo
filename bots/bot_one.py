#This file shows some examples of Bot Type One
...
class BotOne:
    ...
    def stg_momentum(self, price_info, volume_info, time_stamp, moving_avg_period=30):
        price_moving_avg = price_info.rolling(
            window=moving_avg_period).mean().to_list()
        volume_moving_avg = volume_info.rolling(
            window=moving_avg_period).mean().to_list()
    ...
    def stg_mean_reversion(self, price_info, time_stamp, n_std=1, moving_avg_period=30):
        """
                ...
                Rule2 [Surplus] -> trade the order with maximum surplus:
                        ...
                        Sell: lowest buy order, return -1, -0.8, -0.6, -0.4, -0.2
        """
        moving_avg = price_info.rolling(
            window=moving_avg_period).mean().to_list()[time_stamp]
        moving_std = price_info.rolling(
            window=moving_avg_period).std().to_list()[time_stamp]
    ...
    def stg_donchian_breakout(self, price_info, time_stamp, moving_avg_period=30):
        if time_stamp == 0:
            ...
            moving_high = price_info.rolling(
                window=moving_avg_period).max().to_list()[time_stamp-1]
            moving_low = price_info.rolling(
                window=moving_avg_period).min().to_list()[time_stamp-1]
            current_price = price_info.to_list()[time_stamp]
    ...
    def evaluator_ma_surplus_accept(self, price_info, time_stamp, ordered_book, st_moving_avg_period=15, lt_moving_avg_period=30):
        ...
        coefficient = self.stg_ma(
            price_info, time_stamp, st_moving_avg_period, lt_moving_avg_period)
        ...
        if score > 0:
            print(
                f"Highest absolute score is {abs(score)}. Bot buys at ${price} for {abs(share)} shares. \n")
            return price, abs(share), score
        elif score < 0:
            print(
                f"Highest score is {abs(score)}. Bot sells at ${price} for {abs(share)} shares. \n")
            return price, abs(share), score
        else:
            return 0, 0, 0
    ...
    def evaluator_momentum_surplus_accept(self, price_info, volume_info, time_stamp, ordered_book, moving_avg_period=30):
        ...
        if len(ordered_book) == 0:
            return 0, 0, 0
        coefficient = self.stg_momentum(
            price_info, volume_info, time_stamp, moving_avg_period)
    ...
    def evaluator_mean_reversion_surplus_accept(self, price_info, time_stamp, ordered_book, moving_avg_period=30, n_std=1):
        ...
        if len(ordered_book) == 0:
            return 0, 0, 0
        coefficient = self.stg_mean_reversion(
            price_info, time_stamp, n_std, moving_avg_period)
    ...
    def evaluator_donchian_breakout_surplus_accept(self, price_info, time_stamp, ordered_book, moving_avg_period=30):
        ...
        if len(ordered_book) == 0:
            return 0, 0, 0
        coefficient = self.stg_donchian_breakout(
            price_info, moving_avg_period, time_stamp)
    ...
    def crazy_bot(self, price_info, time_stamp, ordered_book, share_lower_limit=50, share_upper_limit=200, n_std=1, moving_avg_period=30):
        ...
        if trade_or_take == 0:  # take
            ...
            share = random.randint(
                share_lower_limit, share_upper_limit) * buy_or_sell
        ...
        if share < 0:
            print(
                f"Crazy Bot buys at ${round(price, 2)} for {abs(share)} shares. \n")
            return price, share
        elif share > 0:
            print(
                f"Crazy Bot sells at ${round(price, 2)} for {share} shares. \n")
            return price, share
        else:
